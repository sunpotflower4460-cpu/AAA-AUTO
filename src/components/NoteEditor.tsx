...
export const NoteEditor = ({
  ...
  const saveLabel =
    saveState === 'saving' ? '保存中…' : saveState === 'saved' ? `${copy.saved} / ${copy.savedEn}` : ''

  return (
    <section className="flex min-h-[calc(100svh-42px)] flex-col px-[21px] pb-[34px] pt-[21px] sm:pt-[34px]">
      ...
      <p
        aria-live="polite"
        className={`mt-[13px] min-h-[20px] text-[13px] text-[var(--color-ink-muted)] transition-opacity duration-[300ms] ${
      ...
      <label className="mt-[21px] block">
        ...
        <input
          ...
          className="title-font h-11 w-full border-0 border-b border-[var(--color-line)] bg-transparent px-[4px] text-[21px] leading-tight outline-none sm:text-[24px]"
        />
      </label>
      ...
      <label className="mt-[21px] flex-1">
        ...
        <textarea
          ...
          className="body-font min-h-[58svh] w-full resize-none rounded-[8px] border border-[var(--color-line)] bg-white/70 p-[21px] text-[16px] leading-[1.75] outline-none transition focus:border-[var(--color-indigo)]"
        />
      </label>
    </section>
  )
